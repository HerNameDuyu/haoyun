...
let arr = query.split("&");
let vinUserId = "";
window.localStorage.setItem("vinUserId", "");
arr.forEach((item) => {
...
service.interceptors.response.use(
  (response) => {
    ...
    if (Number(res.status) != 1) {
      ...
      Toast.fail(res.error_msg || res.msg || res.note || "系统异常");
...
class Request {
  ...
  checkStatus(response) {
    ...
    // 服务器错误
    const errorText =
      response && (codeMessage[response.status] || response.statusText);
  ...
  // 文件
  File(url, data, scope) {
    return this.setRequest("post", url, data, scope, true).then(
      this.fileSuccess
    );
  }
